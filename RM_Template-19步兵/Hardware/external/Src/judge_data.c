...
extern volatile int last_ammunition_angle;
...
u8 get_info_from_buf( u8 *temp, uint16_t command_id)
{
	switch( command_id)
	{
		...
		case SHOOT_DATA:
			...
			if( game_info.shoot_data.bulletSpeed > 10 && game_info.shoot_data.bulletType == SHELL_17MM)		//保存射速
				Shoot_Data_Save(game_info.shoot_data.bulletSpeed);
			
			printf("freq:%5d,speed:%5f\r\n",game_info.shoot_data.bulletFreq,game_info.shoot_data.bulletSpeed);
