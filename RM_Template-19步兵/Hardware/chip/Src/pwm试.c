...
static uint16_t PWM_GPIO_Register;						//pwm gpio注册表
...
//	//GPIO口重复使用检测
//	if( PWM_GPIO_Register_Read( pwm_gpio) == SET)
//	{
//		PWM_GPIO_Register_Set( pwm_gpio);
//	}
//	else
//	{
//		///日志，后续加入
...
void PWM_Set( u32 pwm_gpio, u32 scale)
{
	...
	switch( pwm_gpio / 10000 % 10)
	{
		case 2:tim_setcompare[ pwm_gpio / 1000 % 10 - 1]( TIM2, scale);break;/*printf(" pwm_gpio / 1000 %% 10 - 1 = %d,scale=%d\r\n", pwm_gpio / 1000 % 10 - 1,scale);*/
...
void PWM_GPIO_Register_Set( u32 pwm_gpio)
{
	PWM_GPIO_Register |= ( pwm_gpio / 100000);
}


u8 PWM_GPIO_Register_Read( u32 pwm_gpio)
{
	return PWM_GPIO_Register & ( pwm_gpio / 100000) > 0 ? SET : RESET;
}
