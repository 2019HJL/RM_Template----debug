...
//PWM初始化，参数：外部PWM所用引脚，pwm频率，pwm重装载值，pwm输出极性
void PWM_Init( u32 pwm_gpio, u32 rate, u32 arr, u16 polarity)
{
	...
	else if( pwm_gpio >= PWM_PH10 && pwm_gpio <= PWM_PI0)					//PH10~PH12,PI0
	{
		...
		GPIO_PinAFConfig( pwm_gpio / 100 % 10 == 3? GPIOH: GPIOI, GPIO_PinSource0 + pwm_gpio % 100, GPIO_AF_TIM5);
	}
	else if( pwm_gpio >= PWM_PI5 && pwm_gpio <= PWM_PI2)					//PI5~PI7,PI2(高级定时器)
	{
		RCC_APB2PeriphClockCmd( RCC_APB2Periph_TIM8, ENABLE);
		GPIO_PinAFConfig( GPIOI, GPIO_PinSource0 + pwm_gpio % 100, GPIO_AF_TIM8);
	}
	...
	else if( pwm_gpio / 10000 == 8)
	{
		...
		TIM_CtrlPWMOutputs( TIM8, ENABLE);			//高级定时器主输出使能（须在TIM_Cmd之前）
		TIM_Cmd( TIM8, ENABLE);
...
void PWM_Set( u32 pwm_gpio, u32 scale)
{
	...
	{
		case 2:tim_setcompare[ pwm_gpio / 1000 % 10 - 1]( TIM2, scale);break;
...
void Beep_Freq_Set(int tone_freq)
{
	...
	timer.TIM_Prescaler = (2700.0f/(float)tone_freq*256.0f - 1);;
	timer.TIM_CounterMode = TIM_CounterMode_Up;
	timer.TIM_Period = (128 - 1);
	timer.TIM_ClockDivision = TIM_CKD_DIV1;
