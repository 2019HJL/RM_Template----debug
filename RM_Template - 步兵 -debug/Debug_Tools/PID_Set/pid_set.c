...
void PID_Set(void)// char *ch, u8 len)
{
	...
	//底盘位置环pid
	if( strcmp( pid_set, "PID_Chassis_Pos") == 0) 				
		for( t = 0; t < 4; ++t)
			pid_reset( &chassis_pos_pid[t], kp, ki, kd);	
	
	//云台pitch轴速度环pid
	if( strcmp( pid_set, "PID_Gimbal_Pitch_Spd") == 0) 
		pid_reset( &gimbal_pitch_spd_pid, kp, ki, kd);
	
	//云台pitch轴位置环pid
	if( strcmp( pid_set, "PID_Gimbal_Pitch_Pos") == 0) 
		pid_reset( &gimbal_pitch_pos_pid, kp, ki, kd);		
	
	//云台yaw轴速度环pid
	if( strcmp( pid_set, "PID_Gimbal_Yaw_Spd") == 0) 
		pid_reset( &gimbal_yaw_spd_pid, kp, ki, kd);	
	
	//云台yaw轴位置环pid
	if( strcmp( pid_set, "PID_Gimbal_Yaw_Pos") == 0) 
